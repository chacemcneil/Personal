...
h3 {
  padding: 10px;
  font-weight: 800;
  background-image: linear-gradient(30deg, #00868B, #00C5CD, #00E5EE, #00F5FF, white, white, white, white, white);
  font-size: 20px;
}
h4 {
  text-decoration: underline;
...
* <code>\<a href="https://www.google.com" title ="Google's Homepage"\>This link\</a\></code> <a href="https://www.google.com" title ="Google's Homepage">This link</a> is the same thing created with an html tag.
...
Inline formulas are created by starting and ending with \$ or surrounding with escaped parentheses. Example: $x + 4 = 6$ or \(x+4 = 6\). Block formulas are created using \$\$, as in 
$$ x + 3 = 10$$ 
More complex formulas can also be made 
$$x = {-b \pm \sqrt{b^2-4ac} \over 2a}.$$
as well as multi-line equations 
$$ \begin{array}{} x - 2 & = & {4 \over 2} \times 3 \\ x & = & 2 \times 3 + 2 \\ & = & 8 \end{array}$$
or 
$$ s_{ij} = \left\{ \begin{array}{} -\frac{1}{2} & \text{if} & x_{ij} < 0 \\ 0 & \text{if} & x_{ij} = 0 \\ \frac{1}{2} & \text{if} & x_{ij} > 0 \end{array}\right.$$
...
Sub lists (<code>details</code> parameter):
...
You can also use scrolling to keep tables from using too much space. The following table uses some CSS code to keep the first and last rows visible and alternate row colors.
...
.fixedfront tbody tr td:first-child {
  position: sticky;
  left: 0;
  background: white;
  zIndex: -1;
}
.fixedfront thead tr th:first-child {
  position: sticky;
...
tmp <- rbind(tmp, tmp[, list(Id = "Mean:", Group = "", Value = sprintf("%.2f", mean(as.numeric(Value))))])
htmlTable(tmp, 
          align = "c", 
          rnames = F,                                           # No numbers on rows
          css.class = "fixedheader fixedtotal",                 # The css just before this chunk makes tables of this class have a fixed header
          css.table = "margin: auto;",                          # Center table
          css.cell = "padding-left: 2em; padding-right: 2em;",  # Space on left and right of cells
          total = T                                             # Changes the formatting of the final row
          )
```
</div>

There is also functionality to add a fixed first column. The values in the CSS can be changed to alter this as desired.

```{r, echo = FALSE}
mat <- matrix(1:900, 30, 30)
colnames(mat) <- paste0("Column ", 1:30)
htmlTable(mat, css.class = "fixedheader fixedfront")
```

### Chunk options

Here is a list of convenient chunk options: `r html_list(c("echo (T/F)", "fig.height", "fig.width", "fig.align", "message (T/F)", "warning (T/F)", "results ('asis' interprets output as HTML)"))` Most just control how much extra stuff gets thrown into the document. The chunk for the following graph includes <code>fig.width = 14</code> and <code>fig.height = 3</code> to change the figure size.
...
At this point you can insert actual <code>\<div\></code> tags with the correct id.
...
#### Dropdown menu sections within loop

```{r, echo = FALSE}
## Functions for creating and printing HTML tags
print.html_tag <- function(x) {
  if(!is.null(names(x$params$style))) {
    style_names <- names(x$params$style)
    x$params$style <- paste0(sapply(seq_along(style),
                                    function(i) 
                                      paste0(style_names[i], ": ", style[[i]], ";")),
                             collapse = " " )
  }
  if(length(x$params$class) > 1)
    x$params$class <- paste(x$params$class, collapse = " ")
  if(length(x$innerHTML) > 1)
    x$innerHTML <- paste0(sapply(x$innerHTML, as.character), collapse = "")
  
  lst <- x$attr
  lst <- lst[!sapply(lst, is.null)]
  if(length(lst) > 0)
    lst <- paste0(names(lst), '="', lst, '"')
  lst <- paste(c(x$tag, lst), collapse = " ")
  
  open <- sprintf("<%s%s>", lst, ifelse(x$hidden, " hidden", "") )
  close <- ifelse(x$close, paste0("</", x$tag, ">"), "")
  
  res <- paste0(open, x$innerHTML, close)
  class(res) <- c("htmlTable", "character")
  res
}
as.character.html_tag <- print.html_tag

htag <- function (tag = "div", ..., attr = list(), hidden = F, closetag = T) {
  innerHTML <- list(...)
  if(!is.null(names(innerHTML))) {
    attr <- c(attr, innerHTML[names(innerHTML) != ""])
    innerHTML <- innerHTML[names(innerHTML) == ""]
  }
  ...
  lst <- list(tag = tag,
              ...
              closetag = closetag,
              innerHTML = innerHTML )
  class(lst) <- c("html_tag")
  lst
}

```

To create sections within a loop (or <code>apply</code> function), the same HTML elements must be made within the R chunk. Here that is done by using the <code>knitr</code> function <code>opts_current$get</code> to grab the plots created and embed them within object to be interpretted as HTML.

```{r, results='asis', fig.show="hide", fig.height = 3, fig.width = 6}
## Set up sample data
labels <- paste("Group", LETTERS[1:10])
div_ids <- letters[1:10]
n <- 200

dat <- data.table(expand.grid(Category = labels, Index = 1:n))
dat[, Value := rnorm(.N, 5)]
dat[, Group := sample(1:3, .N, replace = T)]

chunk_plot <- function(num) {
  sprintf("![](%s%s-%s.png)", opts_current$get("fig.path"), opts_current$get("label"), num)
}

## Using loop (option 1)
# for(i in seq_along(labels)) {
#   tab <- htmlTable(dat[Index == i, .(Count = .N, MeanVal = round(mean(Value), 1)), keyby = .(Group)], rnames = F, align = "lr", css.cell = "padding-left: 2em; padding-right: 2em;")
#   invisible(print(ggplot(dat[Category == labels[i]], aes(Value)) + geom_histogram(binwidth = 0.2)))
#   html <- htag("div", id=div_ids[i], hidden = i>1, tab, "This is a random paragraph placeholder....hopefully followed by a graph.",
#                sprintf("![](%s%s-%s.png)", opts_current$get("fig.path"), opts_current$get("label"), i) )
#   fullhtml <- paste0(fullhtml, html, sep = "\n")
# }

## Using lapply function (option 2)
fullhtml <- do.call(htag, c(tag="div", lapply(seq_along(div_ids), function(i) {
  tab <- dat[Index == i, .(Count = .N, MeanVal = round(mean(Value), 1)), keyby = .(Group)]
  tab <- htmlTable(tab, rnames = F, align = "lr", css.cell = "padding-left: 2em; padding-right: 2em;")
  invisible(print(ggplot(dat[Category == labels[i]], aes(Value)) + geom_histogram(binwidth = 0.2)))
  htag("div", tab, id=div_ids[i], hidden = i>1, chunk_plot(i))
})))

## HTML elements can be printed inside the chunk using cat/print functions, or outside the chunk using inline R code, as shown below.
# cat(html_dropdown("test", div_ids, labels))
# print(fullhtml)
```
`r html_dropdown("test", div_ids, labels)`
`r print(fullhtml)`

### Radio buttons

Similar to a dropdown, sometimes you want to toggle certain options which have may (but don't have to) have a smaller effect. The example below shows counts in a table and allows the user to look at row percentages or column percentages instead.

```{r, echo = FALSE}
## A different kind of functions for creating HTML tags in character form
html_tag <- function(tag, .inner = "", .closetag = paste0("</", tag, ">"), .ashtml = T, .tagpad = "\n", .collapse = "\n", ...) {
  params = c(...)
  if(sum(names(params) != "") < length(params)) {
    stop("All tag parameters must be named")
  }
  # params <- gsub('"', "'", params) ## Single quotes are ending up as &#39; ...? Workaround: don't enter quotations inside parameters
  params <- ifelse(is.null(params), "", paste0(" ", names(params), '="', params, '"', collapse = ""))
  
  ## Remove equal sign from boolean parameters
  for(bool in c("checked", "disabled", "hidden", "multiple", "muted", "open", "selected")) {
    params <- gsub(paste0(bool, '=\"TRUE\"'), bool, params)
    params <- gsub(paste0(bool, '=\"FALSE\"'), "", params)
  }
  
  ## Collapse all
  # if(!is.null(.collapse)) {
    .inner <- paste(.inner, collapse = .collapse)
  # }
  html <- paste0("<", tag, params, ">", .tagpad, .inner, .tagpad, .closetag)
  if(.ashtml)
    class(html) <- "htmlTable"
  html
}
```

```{r, echo = FALSE}
html_radio <- function(id, divs, labels = NULL, onchange = NULL, onload = NULL, checked = divs[1]) {
  if(is.null(labels)) {
    if(any(names(divs) == ""))
      stop("Must supply labels or a named vector for div tags.")
    else labels <- names(divs)
  }
  if(is.null(onload)) {
    onload <- glue::glue('{id}_onload()')
  }
  if(is.null(onchange))
    onchange <- glue::glue("{id}_onchange()")
  java <- html_tag("script", glue::glue('
    function {id}_onchange () {{
      var radioButtons = document.getElementsByName("{id}")
      for(var i=0; i<radioButtons.length; i++) {{
        if(radioButtons[i].checked == true) {{
          document.getElementById(radioButtons[i].value).hidden = false;
        }} else {{
          document.getElementById(radioButtons[i].value).hidden = true;
        }}
      }}
    }}
    //window.onload = function () {{
    //  document.getElementById("{checked}_id").checked = true;
    //}}
  '))
  buttons <- sapply(seq_along(divs), function(i) html_tag("input", .inner = labels[i], type="radio", name=id, id=paste0(divs[i], "_id"), value=divs[i], .closetag = "<br>", checked=divs[i]==checked))
  # buttons <- html_tag("input", labels, type="radio", name=id, id=paste0(divs, "_id"), value=divs, .closetag = "<br>", checked=divs==checked, .collapse = NULL)
  form <- html_tag("form", buttons, onchange = onchange, onload = onload, .ashtml = F)
  html <- paste(java, form, sep = "\n")
  browser()
  html
}
```

`r html_radio("rad", c("count", "row_pcnt", "col_pcnt), c("Table of counts", "Percentages by row", "Percentages by column"))`

<div id="count">
```{r, echo = FALSE}
set.seed(40)
n <- 1000
dt <- data.table(Group = sample(LETTERS[1:5], n, replace = T), Category = sample(paste0("Category&nbsp;", 1:7), n, replace = T))
tab <- dt[, table(Category, Group)]
htmlTable(tab, align = "lr", css.cell = padding(2))
```
</div>

<div id="row_pcnt" hidden>
```{r, echo = FALSE}
rowtab <- tab
for(i in 1:nrow(tab)) {
  rowtab[i,] <- percent(tab[i,]/sum(tab[i,]))
}
colnames(rowtab) <- colnames(tab)
row.names(rowtab) <- row.names(tab)
htmlTable(rowtab, align = "lr", css.cell = padding(2))
```
</div>

<div id="col_pcnt" hidden>
```{r, echo = FALSE}
coltab <- tab
for(i in 1:ncol(tab)) {
  coltab[,i] <- percent(tab[,i]/sum(tab[,i]))
}
colnames(coltab) <- colnames(tab)
row.names(coltab) <- row.names(tab)
htmlTable(coltab, align = "lr", css.cell = padding(2))
```
</div>
...
Notice that when you are not at the top of the page, there is a button in the upper right that will return the view to the top of the page.
...
<div class="mySlides" style="border: solid 1px #000; display: table; text-align: center;">
<div style="font-size: 18pt; color: #44B; display: table-cell; vertical-align: middle; height: 300px; padding: 40px;">
Click the arrows on the sides to move between slides.

Note that this first slide is not given the same dimensions, so the window shifts based on the size of each individual display.
</div>
</div>
...
Other markdown files can also be linked to just like web pages. If this file was compiled here, [this link](newfile.html) will point to the HTML file shown above.

### Embedding PDF files

PDF files can also be embedded in an HTML document. In this section of the markdown, a PDF file is downloaded from CRAN and stored in the directory of the file. The file is then embedded in this HTML file so that copies of this file will still show the PDF contents without having to copy the PDF file as well. (This does not work for the method of embedding HTML files shown above.)

```{r, echo = FALSE}
embed_file <- function(fl, filetype = c("pdf", "html", "png", "gif"),
                           width = "100%", height = "400px") {
  filetype <- match.arg(filetype)
  srctype <- switch(filetype,
                    pdf = "application/pdf",
                    html = "text/html",
                    png = "image/png",
                    gif = "image/gif")
  filesize <- file.size(fl)
  if(is.na(filesize))
    filesize <- 1e5
  
  if(filetype == "pdf") {
    b64 <- base64Encode(readBin(fl, "raw", filesize), "txt")
    html <- paste0('<iframe src="data:', srctype, ';base64,', b64,
                   '" style="height: ', height, '; width: ', width, ';"></iframe>')
  # } else if(filetype == "html") {
  #   b64 <- base64Encode(readBin(fl, "raw", filesize), "txt")
  #   html <- paste0('<iframe src="data:', filetype, ';base64,', b64,
  #                  '" style="height: ', height, '; width: ', width, ';"></iframe>')
  } else if(filetype == "png") {
    b64 <- base64Encode(readBin(fl, "raw", filesize), "txt")
    html <- paste0('<image src="data:', filetype, ';base64,', b64,
...
if(!"RColorBrewer.pdf" %in% dir())
  download.file("https://cran.r-project.org/web/packages/RColorBrewer/RColorBrewer.pdf",
                destfile = "RColorBrewer.pdf", mode = "wb")

embed_file("RColorBrewer.pdf")
```
...
After inserting the code above, a <code>\<button\></code> element must be created and linked to a <code>\<div\></code> element. The code below gives an example of how to create a collapsed section (it is not necessary to have the button wrapped in a heading tag). If you want the section expanded by default, replace "+" with "-" in the innerHTML of the <code>button</code> tag. Notice that the ids of the two elements are very similar. The function defined above depends on this similarity to collapse the correct section when the button is clicked.

```{r, eval = FALSE, echo = TRUE}
<div>
<h4><button class="collapsebtn">-</button>Title</h4>
...
<div>
<h4><button class="collapsebtn">+</button>Scatterplot</h4>
```{r, echo = FALSE}
n <- 100
...
<div>
<h4><button class="collapsebtn">+</button>Histogram</h4>
```{r, echo = FALSE}
...
<div>
<h4><button class="collapsebtn">+</button>Density</h4>
```{r, echo = FALSE}
...
<div>
<h4><button class="collapsebtn">-</button>Table</h4>
```{r, echo = FALSE}
...
ggplotly(ggplot(USArrests, aes(Murder, Assault)) + geom_point(), width = 900, height = 400)
plot_ly(USArrests, x = ~State, y = ~Murder, width = 900, height = 400)
```
...
clock <- data.table(expand.grid(Hour = 1:3, Minute = 0:59, Second = 0:59))
...
# if(interactive()) {
    ggplotly(
      ggplot(clock[Hour == 3 & Minute %in% 30:32]) +
        ...
        geom_text(data = clock[Hour == 1 & Second == 0 & Minute %in% (5*(0:11)) &
                                 Hr_Hnd_x + Hr_Hnd_y != 0],
                  aes(Min_Hnd_x, Min_Hnd_y, label = ifelse(Minute == 0, 12, Minute/5))) +
...
Animated plots can also be created using the <code>animation</code> package. **The plots below rely on the file being compiled in its current location to access externally stored graphs.** The package depends on the <code>magick</code> package, which may not be available in all environments. If this is the case, change the first line in the following R chunk to <code>animation_included = FALSE</code>

```{r, echo = FALSE, warning = FALSE, message = FALSE, results = 'asis'}
animation_included = T

if(animation_included) {
  script <- paste('<link rel="stylesheet" href="css/scianimator.css" />',
                  '<script src="js/jquery.scianimator.min.js"></script>', sep = "\n")
  class(script) <- "html"
  script
}
```
...
if(animation_included) {
  ...
  saveHTML({
    for (i in 0:(nrow(dt)-1)) {
      dt[ind <= i, {
        plot(x, y, type = "l", xlim = range(dt$x), ylim = range(dt$y), main = "Random Walk")
        points(x[i+1], y[i+1])
        points(0, 0, pch = 19)
      }]
    }
...
if("ggsn" %in% row.names(installed.packages())) {
  scalefun <- ggsn::scalebar
} else {
  scalefun <- function(...) {return(NULL)}
  print("Package ggsn is not installed. Install package to change scalebar format of chloropleths")
}
...
plot_usmap(data = dt1, values = "Population", labels = T)+
  ...
  scale_fill_gradientn(colours=rev(heat.colors(10)),na.value="grey90",
                       guide = guide_colourbar(barwidth = 25, barheight = 0.4,
                                               ...
                                               title.position = "top")) +
  # map scale
  scalefun(data = statemap, dist = 500, dist_unit = "km",
...
<table><tr><td style="width: 20%;"><a href="https://d3js.org/" target="_blank">`r embed_file("https://raw.githubusercontent.com/d3/d3-logo/master/d3.png", filetype = "png", width = "60px", height = "60px")`</a></td><td>
There are a lot of different graphs and figures that can be made with the D3 javascript library. It is sometimes tricky to implement this in R markdown, but it can be done.
</td></tr></table>

### Shiny

```{r, echo = FALSE}
src <- paste0("https://community.rstudio.com/uploads",
              "/default/original/1X/c017cef9c13bc",
              "937df73659d3b5b1411a39c7ed2.png")
src <- paste0("https://d33wubrfki0l68.cloudfront.net/",
              "d08c35bfe1ba68584788f5cdcad703d0c9d8263f/",
              "7969a/wp-content/uploads/2014/04/shiny.png")
```

<table><tr><td style="width: 20%;"><a href="https://shiny.rstudio.com" target="_blank">`r embed_file(src, filetype = "png", width = "50px", height = "60px")`</a></td><td>
One obvious option for interactively viewing the results of analysis is Shiny. Through a Shiny app, readers can potentially dive deep into the data and even perform some analysis of their own (whether good or bad). 
</td></tr></table>
<br>
<table><tr><td style="width: 50%;">
Pros:
`r html_list(c("Interact with raw data", "Many parameters can be adjusted in any combination", "Does not require a file, only a URL"))`
</td><td>
Cons:
`r html_list(c("Potentially too much information", "Users can go looking for their own desired answers", "Requires an active R session"))`
</td></tr></table>
