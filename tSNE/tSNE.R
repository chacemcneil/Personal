# t-SNE exploration
 ...
 animate <- function(pts, grp, filename, loop = F, navigator = F, interval = .5, epoch = 50, max_iter = 1e3, perplexity = 30, ...) {
   ...
   saveHTML({tsne(pts, 
                  ...
                  epoch_callback = function(x) {
                    #tmp <- data.frame(tmp)
                    print(ggplot(data.frame(x), aes(X1, X2)) + geom_point(col = grp))
                  }, 
                  max_iter = max_iter, 
