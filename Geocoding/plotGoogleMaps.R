# Tweak google map function.
library(plotGoogleMaps)  # Does not work on Riley2

env <- environment(plotGoogleMaps)

plotGoogleMaps <- 
function (SP, filename = "", zcol = 1, at = NULL, add = FALSE, 
          previousMap = NULL, colPalette = NULL, strokeColor = "", 
          strokeOpacity = 1, fillOpacity = 0.7, strokeWeight = 1, geodesic = TRUE, 
          clickable = TRUE, draggableMarker = FALSE, iconMarker = "", 
          flat = TRUE, visible = TRUE, zIndex = "null", map.width = "100%", 
          map.height = "100%", layerName = "", control.width = "100%", 
          control.height = "100%", zoom = 15, fitBounds = TRUE, mapTypeId = "HYBRID", 
          disableDoubleClickZoom = FALSE, draggable = TRUE, keyboardShortcuts = TRUE, 
          mapTypeControlOptions = "DEFAULT", navigationControl = TRUE, 
          navigationControlOptions = "DEFAULT", scaleControlOptions = "STANDARD", 
          noClear = FALSE, scrollwheel = TRUE, streetViewControl = FALSE, 
          legend = TRUE, control = TRUE, InfoWindowControl = list(map = map, 
                                                                  event = "click", position = "event.latLng", disableAutoPan = FALSE, 
                                                                  maxWidth = 330, pixelOffset = "null", zIndex = "null"), 
          map = "map", mapCanvas = "map_canvas", css = "", api = "https://maps.google.com/maps/api/js?sensor=false&v=3.18", 
          openMap = TRUE
          #Additional tweaking parameters
          ,markertextheight = 10, markerscale = .6, printAttribute = T
          ) 
{
  ...
  if ((class(SP)[1] == "SpatialPixelsDataFrame" || class(SP)[1] == 
  ...
  if (class(SP.ll)[1] == "SpatialPointsDataFrame") {
    if (length(iconMarker) < length(SP.ll@coords[, 1])) {
      iconMarker = iconlabels(attribute, colPalette, at, 
                              height = markertextheight, icon = TRUE, scale = markerscale)
    }
    else {
  ...
  if (class(SP.ll)[1] == "SpatialPoints") {
    if (length(iconMarker) < length(SP.ll@coords[, 1])) {
      iconMarker = iconlabels(rep(iconMarker[1], length(SP.ll@coords[,1])), colPalette, at, 
                              height = markertextheight, icon = TRUE, scale = markerscale)
    }
    else {
      iconMarker = iconMarker[1:length(SP.ll@coords[, 1])]
  ...
  if (!is.list(previousMap)) {
    ...
    init <- createInitialization(SP.ll, add = T, name = map, 
                                 divname = mapCanvas, zoom = zoom, fitBounds = fitBounds, 
                                 mapTypeId = mapTypeId, disableDefaultUI = disableDefaultUI, 
                                 disableDoubleClickZoom = disableDoubleClickZoom, 
                                 draggable = draggable, keyboardShortcuts = keyboardShortcuts, 
                                 mapTypeControlOptions = mapTypeControlOptions, scaleControlOptions = scaleControlOptions, 
                                 navigationControl = navigationControl, navigationControlOptions = navigationControlOptions, 
                                 noClear = noClear, scrollwheel = scrollwheel, streetViewControl = streetViewControl)
  ...
  if (class(SP)[1] == "SpatialPoints") {
    ...
    var1 <- paste(sapply(as.list(1:length(SP.ll@coords[, 1])), 
                         function(i) paste(var1, createMarker(SP.ll@coords[i, ], 
                                                              title = paste(nameOfSP, " NO: ", as.character(i), sep = ""), 
                                                              clickable = clickable, draggable = draggableMarker, 
                                                              flat = flat, visible = visible, map = map, icon = iconMarker[i], 
                                                              zIndex = zIndex), "\n", sep = "")), pointsName, ".push(marker); \n", 
                  sep = "", collapse = "\n")
    var <- paste(var, var1)
    ...
    if (control) {
      endhtm <- paste(endhtm, "<input type=\"checkbox\" id=\"", 
                      boxname, "\" onClick='boxclick(this,", pointsName, 
                      ",\"", boxname, "\",", map, ");' /> <b>", layerName, 
                      "<b> <hr/>", sep = "")
    }
  }
    ...
    pointsName <- paste("markers", nameOfSP, randNum, sep = "")
    boxname <- paste(pointsName, "box", sep = "")
    att <- rep(NA, 0.5 * length(slot(SP.ll, "coords")))
    att1 = ""
    if (!is.list(previousMap)) {
      var <- ""
      var <- paste(" var marker \n var ", map, " \n")
    ...
    att <- paste(lapply(as.list(1:length(SP.ll@coords[, 1])), 
                        function(i) paste(names(SP.ll@data)[k], ":", 
                                          sapply(k, function(k) as.character(SP.ll@data[i, k])), "\\r", collapse = "")))
    var1 <- paste(sapply(as.list(1:length(SP.ll@coords[, 1])), 
                         function(i) paste(var1, createMarker(SP.ll@coords[i, ], title = paste(att[i], sep = ""), 
                                                              clickable = clickable, draggable = draggableMarker, 
                                                              flat = flat, map = map, visible = visible, 
                                                              icon = iconMarker[i], zIndex = zIndex), 
                                           "\n", sep = "")), pointsName, ".push(marker); \n", 
                  sep = "", collapse = "\n")
    att <- paste(lapply(as.list(1:length(SP.ll@coords[, 1])), 
                        function(i) paste(names(SP.ll@data)[k], ":", 
    ...
    infW <- paste(lapply(as.list(1:length(SP.ll@coords[, 1])), 
                         function(i) paste(infW, createInfoWindowEventM(Marker = paste(pointsName, "[", i - 1, "] ", sep = ""), 
                                                                        content = att[i], map = InfoWindowControl$map, 
                                                                        event = InfoWindowControl$event, position = InfoWindowControl$position, 
                                                                        disableAutoPan = InfoWindowControl$disableAutoPan, 
                                                                        maxWidth = InfoWindowControl$maxWidth, pixelOffset = InfoWindowControl$pixelOffset, 
    ...
    if (legend) {
      ...
      pp <- legendbar(cxx$brks, colPalette = cxx$col.uniq, 
                      legendName = divLegendImage, temp = temporary)
    ...
    var1 <- paste(lapply(as.list(1:length(SP.ll@lines)), 
                         function(i) paste(var1, createLine(SP.ll@lines[[i]], 
                                                            strokeColor = xx[i], strokeOpacity = strokeOpacity, 
                                                            strokeWeight = strokeWeight, geodesic = geodesic, 
                                                            clickable = clickable, map = map, zIndex = zIndex), 
                                           "\n", sep = "")), lineName, ".push(line); \n", 
                  sep = "", collapse = "\n")
    var <- paste(var, var1)
    functions <- paste(functions, "showO(", lineName, ",\"", 
                       boxname, "\",", map, ");", sep = "")
    if (!is.list(previousMap)) {
      endhtm <- paste("</script> \n </head> \n <body onload=\"initialize()\"> \n  <div id=\"", 
                      mapCanvas, "\"></div>  \n\n                           \n <div id=\"cBoxes\"> \n", 
    ...
    if (control) {
      endhtm <- paste(endhtm, "<table border=\"0\"> \n <tr> \n  <td> <input type=\"checkbox\" id=\"", 
                      boxname, "\" onClick='boxclick(this,", lineName, 
                      ",\"", boxname, "\",", map, ");' /> ", layerName, 
                      " </td> </tr>  \n", sep = "")
      endhtm <- paste(endhtm, " \n <tr>  <td> \n <input type=\"text\" id=\"", 
    ...
    var1 <- paste(sapply(1:length(SP.ll@lines), 
                         function(i) paste(var1, createLine(SP.ll@lines[[i]], strokeColor = xx[i], 
                                                            strokeOpacity = strokeOpacity, strokeWeight = swxx[i], 
                                                            geodesic = geodesic, clickable = clickable, zIndex = zIndex), 
                                           "\n", sep = "")), lineName, ".push(line); \n", sep = "", 
                  collapse = "\n")
    k = 1:length(names(SP.ll@data))
    ...
    infW <- paste(lapply(as.list(1:length(SP.ll@lines)), 
                         function(i) paste(infW, createInfoWindowEvent(Line_or_Polygon = paste(lineName, 
                                                                                               "[", i - 1, "] ", sep = ""), content = att[i], 
                                                                       map = InfoWindowControl$map, event = InfoWindowControl$event, 
                                                                       position = InfoWindowControl$position, disableAutoPan = InfoWindowControl$disableAutoPan, 
                                                                       maxWidth = InfoWindowControl$maxWidth, pixelOffset = InfoWindowControl$pixelOffset, 
                                                                       zIndex = InfoWindowControl$zIndex), " \n")), 
                  collapse = "\n")
    functions <- paste(functions, infW, "showO(", lineName, 
                       ",\"", boxname, "\",", map, ");", sep = "")
    if (!is.list(previousMap)) {
      endhtm <- paste("</script> \n </head> \n <body onload=\"initialize()\"> \n  <div id=\"", 
                      mapCanvas, "\"></div>  \n\n                           \n <div id=\"cBoxes\"> \n", 
    ...
    if (length(colPalette) == length(SP.ll@polygons)) {
      ...
      cxx <- PolyCol(1:length(SP.ll@polygons), colPalette, 
                     at = at)
      xx <- cxx$cols
    }
    ...
    var1 <- paste(lapply(as.list(1:length(SP.ll@polygons)), 
                         function(i) paste(var1, createPolygon(SP.ll@polygons[[i]], 
                                                               fillColor = xx[i], strokeColor = strokeColor, 
                                                               strokeOpacity = strokeOpacity, strokeWeight = swxx[i], 
                                                               geodesic = geodesic, map = map, clickable = clickable, 
                                                               fillOpacity = fillOpacity, zIndex = zIndex), 
                                           "\n", sep = "")), polyName, ".push(polygon); \n", 
                  sep = "", collapse = "\n")
    var <- paste(var, var1)
    ...
    if (control) {
      endhtm <- paste(endhtm, "<table border=\"0\"> \n <tr> \n  <td> <input \n                            type=\"checkbox\" id=\"", 
                      boxname, "\" onClick='boxclick(this,", polyName, 
                      ",\"", boxname, "\",", map, ");' /> ", layerName, 
                      " </td>  </tr> \n", sep = "")
      endhtm <- paste(endhtm, " \n <tr> <td> \n <input type=\"text\" id=\"", 
    ...
    att <- paste(lapply(as.list(1:length(SP.ll@polygons)), 
                        function(i) paste(names(SP.ll@data), ":", sapply(k, 
                                                                         function(k) as.character(SP.ll@data[i, k])), 
                                          "<br>", collapse = "")))
    var <- paste(var, var1)
    infW <- ""
    infW <- paste(lapply(as.list(1:length(SP.ll@polygons)), 
                         function(i) paste(infW, createInfoWindowEvent(Line_or_Polygon = paste(polyName, 
                                                                                               "[", i - 1, "] ", sep = ""), content = att[i], 
                                                                       map = InfoWindowControl$map, event = InfoWindowControl$event, 
                                                                       position = SP.ll[i, ]@polygons[[1]]@labpt, disableAutoPan = InfoWindowControl$disableAutoPan, 
                                                                       maxWidth = InfoWindowControl$maxWidth, pixelOffset = InfoWindowControl$pixelOffset, 
                                                                       zIndex = InfoWindowControl$zIndex), " \n")), 
                  collapse = "\n")
    functions <- paste(functions, infW, "showO(", polyName, 
                       ",\"", boxname, "\",", map, ");", sep = "")
    if (!is.list(previousMap)) {
      endhtm <- paste("</script> \n </head> \n <body onload=\"initialize()\"> \n  <div id=\"", 
                      mapCanvas, "\"></div>  \n\n                           \n <div id=\"cBoxes\"> \n", 
    ...
    rasterName <- substr(rasterName, start = regexpr("grid", 
                                                     rasterName), stop = nchar(rasterName))
    ...
    var <- paste(var, "\n var ", rasterName, "imageBounds = new google.maps.LatLngBounds\n                (new google.maps.LatLng(", 
                 sw[1], ",", sw[2], "),\n                new google.maps.LatLng(", 
                 ne[1], ",", ne[2], ")); \n", sep = "")
    var <- paste(var, "var ", rasterName, "= new USGSOverlay(", 
                 rasterName, "imageBounds", ",\"", rasterName, ".png\",", 
                 map, "); \n", sep = "")
    if (is.null(colPalette)) {
      pal <- colorRampPalette(c("green", "orange", "brown"), 
                              space = "Lab")
      xx <- colPalette <- as.character(substr(pal(12), 
                                              1, 7))
    }
    else {
      xx <- colPalette <- as.character(substr(colPalette, 
                                              1, 7))
    }
    if (is.factor(attribute)) {
      ...
      SP$arg1111 <- as.numeric(SP[attributeName]@data[, 
                                                      1])
    }
    else {
      SP$arg1111 <- SP[zcol]@data[, 1]
    }
    cxx = PolyCol(SP[attributeName]@data[, 1], colPalette, 
                  at)
    xx = cxx$cols
    pp <- legendbar(cxx$brks, colPalette = cxx$col.uniq, 
                    legendName = divLegendImage, temp = temporary)
    SGqk <- GE_SpatialGrid(SP.ll)
    png(filename = ifelse(temporary, paste(tempdir(), "/", 
                                           rasterName, ".png", sep = ""), paste(rasterName, 
                                                                                ".png", sep = "")), width = 10 * SGqk$width, height = 10 * 
          SGqk$height, bg = "transparent")
    par(mar = c(0, 0, 0, 0), xaxs = "i", yaxs = "i")
    image(as.image.SpatialGridDataFrame(SP["arg1111"]), breaks = cxx$brks, 
          col = cxx$col.uniq)
    dev.off()
    functions <- paste(functions, "showR(", rasterName, ",\"", 
                       boxname, "\",", map, ");", sep = "")
    if (!is.list(previousMap)) {
      endhtm <- paste("</script> \n </head> \n <body onload=\"initialize()\"> \n  <div id=\"", 
                      mapCanvas, "\"></div>  \n\n                           \n <div id=\"cBoxes\"> \n", 
    ...
    if (control) {
      endhtm <- paste(endhtm, "<table border=\"0\"> \n <tr> \n  <td> <input type=\"checkbox\" id=\"", 
                      boxname, "\" onClick='boxclickR(this,", rasterName, 
                      ",\"", boxname, "\",", map, ");' /> <b> ", layerName, 
                      "<b> </td> </tr> \n", sep = "")
  ...
  if (add == F) {
    functions <- paste(functions, " google.maps.event.addListener( ", 
  ...
  x <- list(starthtm = starthtm, var = var, functions = functions, 
            endhtm = endhtm)
  return(x)
}
environment(plotGoogleMaps) <- env